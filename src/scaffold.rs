  ...
  scan:
    ...
    steps:
      ...
      - uses: actions/setup-node@v4
        with:
          node-version: "20"
      - name: Scan tracked files (structural patterns only)
        run: npx --yes doxguard@0.1.0 scan --all-tracked --block
"#;
...
pub fn install_hooks(cwd: &Path) -> Result<Vec<ScaffoldAction>> {
    if has_husky(cwd) {
        return Ok(vec![ScaffoldAction {
            ...
            status: ActionStatus::Skipped,
            detail: Some(
                "husky detected; add `doxguard scan --staged --block` to the existing hook"
                    .to_owned(),
            ),
        }]);
    }
    let hook = create_without_overwrite(cwd, ".githooks/pre-commit", HOOK_TEMPLATE)?;
    #[cfg(unix)]
